...
module.exports = ({ cache, config }) => {
  ...
  // Helpers
  const proxyPrivateDownload = async (asset) => {
  ...
  exports.download = async (req, res) => {
    ...
    if (shouldProxyPrivateDownload) {
      const assetRes = await proxyPrivateDownload(platforms[platform]);
  ...
  exports.downloadPlatform = async (req, res) => {
    ...
    if (token && typeof token === "string" && token.length > 0) {
      const assetRes = await proxyPrivateDownload(latest.platforms[platform]);
      res.setHeader("Location", assetRes.headers.get("Location"));
      send(res, 302);
      return;
    }

    console.log("redirecting download: " + latest.platforms[platform].url);
    res.writeHead(302, {
  ...
  exports.update = async (req, res) => {
    ...
    if (compare(latest.version, version) !== 0) {
      ...
      // TODO: support public repos too without token!
      const assetRes = shouldProxyPrivateDownload && await proxyPrivateDownload(asset);

      const sigAsset = latest.platforms[`${platform}_sig`];

      let signature = '';
      if (sigAsset) {
        // TODO: support public repos too without token!
        const sigAssetRes = shouldProxyPrivateDownload && await proxyPrivateDownload(sigAsset);
        const sigRes = await fetch(sigAssetRes.headers.get("Location"), {
          headers: { Authorization: `Bearer ${token}` },
        });
        signature = sigRes && (await sigRes.text());
      }
      ...
      send(res, 200, {
        ...
        url: shouldProxyPrivateDownload
          ? assetRes && assetRes.headers.get("Location")
